...
def _manager(db, tmp_path, **kwargs):
    return QueueManager(
...
def test_enqueue_and_complete_single_download(db, tmp_path):
...
def test_history_recorded_on_completion(db, tmp_path):
    mgr = _manager(db, tmp_path)
...
def test_concurrency_limit_respected(db, tmp_path):
    backend = MockBackend(chunk_size=64, total_size=4096)  # slow enough to observe overlap
    mgr = _manager(db, tmp_path, backend=backend, max_concurrent=2)
...
def test_duplicate_url_not_enqueued_twice_while_active(db, tmp_path):
    backend = MockBackend(chunk_size=8, total_size=100_000)  # slow, stays active
    mgr = _manager(db, tmp_path, backend=backend, max_concurrent=1)
...
def test_skip_duplicate_already_in_history(db, tmp_path):
    backend = MockBackend()
    mgr = _manager(db, tmp_path, backend=backend, duplicate_behavior="skip")
    ...
    mgr2 = _manager(db, tmp_path, backend=backend, duplicate_behavior="skip")
    mgr2.enqueue_url("https://youtube.com/watch?v=other-alias-1")
    assert mgr2.wait_idle(timeout=5)
...
def test_pause_and_resume(db, tmp_path):
    backend = MockBackend(chunk_size=32, total_size=20000)
    mgr = _manager(db, tmp_path, backend=backend, max_concurrent=1)
...
def test_cancel_removes_partial_file(db, tmp_path):
    backend = MockBackend(chunk_size=32, total_size=50000)
    mgr = _manager(db, tmp_path, backend=backend, max_concurrent=1)
...
def test_failed_extraction_marks_item_failed(db, tmp_path):
    backend = MockBackend(fail_urls={"https://youtu.be/willfail1234"})
    mgr = _manager(db, tmp_path, backend=backend)
...
def test_retry_failed_item(db, tmp_path):
...
def test_remove_item(db, tmp_path):
    mgr = _manager(db, tmp_path)
...
def test_no_double_dispatch_of_same_item(db, tmp_path):
    ...
    backend = MockBackend(chunk_size=4, total_size=2000, force_title="Same Title Every Time")
    mgr = _manager(db, tmp_path, backend=backend, max_concurrent=4)
...
def test_retry_all_failed_with_forced_title_collision_no_corruption(db, tmp_path):
...
def test_reorder(db, tmp_path):
    backend = MockBackend()
    mgr = _manager(db, tmp_path, backend=backend, max_concurrent=0)  # nothing auto-starts
...
def test_events_emitted(db, tmp_path):
    events = []
    backend = MockBackend()
    mgr = QueueManager(
        queue_repo=QueueRepository(db), history_repo=HistoryRepository(db), backend=backend,
        download_folder=str(tmp_path / "downloads"), max_concurrent=2,
...
def test_shutdown_joins_active_threads_and_preserves_part_file(db, tmp_path):
    ...
    backend = MockBackend(chunk_size=8, total_size=200_000)
    mgr = _manager(db, tmp_path, backend=backend, max_concurrent=1)
    ...
    backend = MockBackend(chunk_size=16, total_size=8000, force_title="Highlights")
    mgr = _manager(db, tmp_path, backend=backend, max_concurrent=2)
